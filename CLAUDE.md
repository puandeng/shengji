...
- **Scoring:** only the attacking team accumulates points. Defending team never scores — they only deny. The trump-calling team is the defender; winning the last trick "protects" the kitty (no bonus). When the non-caller team wins the last trick, kitty points × (2 × cards in the winning play) are added to the score — single ×2, pair ×4, tractor of 3 pairs ×12.
...
| Kitty | Trump caller (defender) winning the last trick "protects" the kitty (no bonus). Non-caller winning → kitty pts × (2 × cards in winning play) added to score |
...
| Jack demotion | If attackers are at level J and defenders win the last trick with a Jack card, attackers are demoted to level 2 |
