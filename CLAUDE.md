...
- **Scoring:** the attacking team (non-caller) accumulates points from tricks they win. The defending team (trump caller) tries to deny them. When the defending team wins the last trick, the kitty is "protected" (no bonus). When the attacking team wins the last trick, kitty points × (2 × cards in the winning play) are added to the score — single ×2, pair ×4, tractor of 3 pairs ×12.
...
| Kitty | Defending team (caller) winning the last trick "protects" the kitty (no bonus). Attacking team winning → kitty pts × (2 × cards in winning play) added to score |
