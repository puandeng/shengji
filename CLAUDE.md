...
- **Scoring — read this carefully, the names lie.** `attackingTeam` is set to the **trump caller's** team (`GameState.callTrump`), and that same team accumulates points and chases the threshold. Traditionally it is the other way round: the caller's team are the declarers, who hold the kitty and *deny*, while the opposing team collects. Swapping the semantics to match tradition is an open item in `PLAN.md`; until it lands, read `attackingTeam` as "the trump caller's team, which collects points here".
- **The kitty must never pay the team that buried it.** The declarer chooses the 8 buried cards *and* collects points, so crediting the kitty to them makes burying Kings free profit. Buried points are a liability: winning the last trick merely protects them (no gain — they were your own), and losing it lets the opposing team capture the bury, costing kitty points × (2 × cards in the winning play), floored at 0.
...
| Kitty | Buried by the declarer, never pays them. Attackers winning the last trick protects it (no gain); losing it costs them kitty pts × (2 × cards in the winning play), floored at 0 |
